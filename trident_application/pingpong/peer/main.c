...
int main(int argc, char* argv[]) {
    ...
    if(8 > argc || (8 != argc && 10 != argc)) {
        fprintf(stderr, "--usage: bitwise_ch_alteration <ch0 tx> <ch0 rx> <ch0 data> <ch1 tx> <ch1 rx> <ch1 data> <bit width> [<reset pin nr> <reset response pin nr>]\n");
        return finalize(-1);
    } else if(10 == argc) {
    ...
    BIT* word = (BIT*) calloc(chout.bit_width, sizeof(BIT));
    ...
    char word_string[chout.bit_width + 1];

    for(unsigned int i = 0; i < chout.bit_width; ++i) {
        word_string[i] = word[i] + '0';
    }
    word_string[chout.bit_width] = '\0';
