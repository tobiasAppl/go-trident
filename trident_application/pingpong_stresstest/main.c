...
int main(int argc, char* argv[]) {
    ...
    if(7 > argc || (7 != argc && 9 != argc)) {
        fprintf(stderr, "--usage: pingpong_benchmark <ch0 tx> <ch0 rx> <ch0 data> <ch1 tx> <ch1 rx> <ch1 data> [<reset pin nr> <reset response pin nr>]\n");
        return finalize(-1);
    } else if(9 == argc) {
    ...
    if(-1 == chout.channel_nr) {
        ERROR(-1, "Something unexpected went wrong during initialization, output channel nr not set");
        return finalize(-1);
    }
    if(-1 == chin.channel_nr) {
        ERROR(-1, "Something unexpected went wrong during initialization, input channel nr not set");
        return finalize(-1);
    }
    ...
    while(1) {
        ...
        sRes = layer1_write_to_channel(chout.channel_nr, word_out, 0);
        if(sRes != ERRNO_NO_ERROR) {
            ERROR(sRes, "Sending failed");
            return finalize(sRes);
        }
        rRes = layer1_read_from_channel(chin.channel_nr, word_in, 0);
        if(rRes != ERRNO_NO_ERROR) {
